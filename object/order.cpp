...
OrderTable::OrderTable(const string& file_dir) {
    ...
    while (fs.good()) {
        ...
        fs >> order_id >> latitude >> longitude
        >> terminal_id >> dest_id >> cbm
        >> from >> to >> load
        >> temp >> temp >> group; // date, group, whole_group

        table[group].emplace_back(order_id, terminal_id, dest_id,
...
Order::Order(const string &orderId, const string &terminalId, const string &destId,
